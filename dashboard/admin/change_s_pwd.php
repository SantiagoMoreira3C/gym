...
if (isset($user_id_auth) && isset($pass) && isset($key)) {
    ...
    if ($count == 1) {
        ...
        echo "<html><head><script>alert('Su perfil ha sido actualizado, para ingresar debe volver a iniciar sesión ');</script></head></html>";
        echo "<meta http-equiv='refresh' content='0; url=logout.php'>";
    } else {
        echo "<html><head><script>alert('Su cambio no se ha podido realizar');</script></head></html>";
        echo "error".mysqli_error($con);
        
    }
} else {
    echo "<html><head><script>alert('Cambio no realizado');</script></head></html>";
    echo "error".mysqli_error($con);
 
}
}
else{
    echo "<html><head><script>alert('Sus contraseñas no coinciden: Una de las dos esta incorrecta');</script></head></html>";
